...
/**
 * Centralized registry of audit `action` strings. Every call to
 * `logAuditEvent` should reference one of these (directly or by convention) so
 * the viewer's action filter and the documentation stay in sync. New actions
 * are added here only.
 */
...
/**
 * Canonical `entityType` values. The viewer validates the `entityType` filter
 * against this set so only real entity types are queryable.
 */
