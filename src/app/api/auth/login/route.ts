...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = NextResponse.json({
      success: true,
      data: { id: user.id, email: user.email, role: user.role, businessId: user.businessId },
    });
    setSessionCookie(response, session.token, session.expiresAt);
    await logAuditEvent({ action: 'LOGIN_SUCCESS', entityType: 'Auth', entityId: user.id, userId: user.id, businessId: user.businessId, description: 'User logged in', ipAddress });
    return response;
  } catch (error) {
